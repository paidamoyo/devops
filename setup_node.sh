...
function setup_puppet {
  ...
  wget -qN http://apt.puppetlabs.com/puppetlabs-release-${codename}.deb
...
function setup_monitor {
  if [[ ! -f /root/provisioned.txt ]]; then
    ...
    wget -qN $DOWNLOAD_URL/nagios.tgz
    tar zxvf nagios.tgz

    wget -qN $DOWNLOAD_URL/cucumber.tgz
    ...
    apt-get install --yes git build-essential libxml2-dev libxslt-dev
    ...
    gem install ./cucumber-nagios-0.9.3.gem --no-ri --no-rdoc --conservative
    cd -
    bundle update # updating Gemfile.lock with latest dependencies brought in by cucumber-nagios
